# LESSONS.md — MediVasc website

Durable lessons the advisor folds in after each review. `AGENTS.md` carries the
one-line index; the full bodies live here. Read only the ones relevant to what
you're touching. The two-writer / never-force rule's canonical statement is the
STOP block in `AGENTS.md`, not a lesson here.
...
## From spec 004 (admin validation sync — first Codex-executed spec, signed off 2026-07-13)
...
## From the design/mobile/email bundle + custom-domain migration (advisor-implemented, 2026-07-13)
...
19. **`main` has two writers — the advisor syncs before every write.** The admin
    ...
    **never `--force`.** Treat `data/*.json` as the admin's domain: avoid
    ...
    → keep `[]` → rebuild resolved it with zero owner content lost. (The canonical
    short form of this rule is the STOP block at the top of `AGENTS.md`.)
